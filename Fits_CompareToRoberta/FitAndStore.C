...
void FitAndStore()
{
  ...
  TFile *inputFile = new TFile("fRobHistoEvMix.root");
  ...
  for (int iCentBin = 0; iCentBin < numberOfCentRanges; iCentBin++)
  {
    for (int iRapBin = 0; iRapBin < numberOfRapRanges; iRapBin++)
    {
      for (int iPtBin = 0; iPtBin < numberOfPtRanges; iPtBin++)
      {
        ...
        TString plotsPath;
        plotsPath.Form("FitPlots_Roberta/Cent-%gto%g/Pt-%gto%g/Rap-%gto%g", centRanges[iCentBin][0], centRanges[iCentBin][1], ptRanges[iPtBin][0], ptRanges[iPtBin][1], rapRanges[iRapBin][0], rapRanges[iRapBin][1]);
