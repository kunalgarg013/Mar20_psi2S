...
Bool_t AliAnalysisMuonUtility::SetSparseRange(THnSparse* hSparse,
                                              Int_t ivar, Double_t varMin, Double_t varMax,
                                              const char* option)
{
  //
                                              ...
                                              Int_t ivar, const char* labelName)
{
  ...
  Int_t ibin = axis->FindBin(labelName);
  if ( ibin < 1 ) {
    printf("Warning: %s: label %s not found. Nothing done", hSparse->GetName(), labelName);
    return kFALSE;
  }
  return SetSparseRange(hSparse,ivar,ibin,ibin,"USEBIN");
}
                                              ...
                                              const char* option)
{
  ...
  return SetSparseRange(gridSparse->GetGrid(), ivar, varMin, varMax, option);
}
...
{
  //
  /// Sets range using a label name
  //

  return SetSparseRange(gridSparse->GetGrid(), ivar, labelName);
}
