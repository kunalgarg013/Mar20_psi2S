void emptybins()
{
    TFile* f1 = new TFile("rAcceptance_2018.root");
    ...
    cout<<hSubtracted[0]->GetNbinsX()<<endl;
    ...
    for(Int_t p=0; p<10; p++)
    {
        for(Int_t k=1; k<=200; k++)
        {
            ...
            hMixedSubtracted[p]->SetBinError(k, 0);
        }
    ...
    TFile* f2 = new TFile("rACC_Extended.root", "RECREATE");
