...
void subMixing_Rap()
{
    ...
    const Double_t RapLow[]    = {-4,   -3.5,  -3.25, -3,    -2.75};
    const Double_t RapHigh[]   = {-3.5, -3.25, -3,    -2.75, -2.5};
    const Double_t dpT[]      =  {0.5,  0.25,  0.25,  0.25,  0.25};
    const Int_t nCentBins = 5; // no of Centrality bins

    const Int_t RapBins = 5;

    TString strRapLow[]  = {"-4",     "-3.5",  "-3.25",       "-3",    "-2.75"};
    TString strRapHigh[] = {"-3.5",  "-3.25",     "-3",    "-2.75",     "-2.5"};
    ...
    TFile* f2                       = new TFile("rap_Mixed_oqr.root");
    TFile* fNew =    new TFile("scaledRapmixed.root", "RECREATE");
    ...
    for(Int_t k=1; k<5; k++)
    {
        ...
        {
            i= i+1;
            cout<<Form("histoInvMass_Cent%ito%i_Rap%sto%s;1", cLow[k], cHigh[k], strRapLow[l].Data(), strRapHigh[l].Data())<<endl;
            hMixed[i] = (TH1F*)InvMassUnlike->Get(Form("histoInvMass_Cent%ito%i_Rap%sto%s;1", cLow[k], cHigh[k], strRapLow[l].Data(), strRapHigh[l].Data()));
            cout<<Form("histoInvMass_Cent%ito%i_Rap%sto%s;1", cLow[k], cHigh[k], strRapLow[l].Data(), strRapHigh[l].Data())<<"     "<<i<<endl;
    ...
    for(Int_t m=0; m<5; m++)
    {
        ...
        hpTMixed[m]->SetName(Form("Rap_%sto%s", strRapLow[m].Data(), strRapHigh[m].Data()));
        cout<<Form("Rap_%sto%s", strRapLow[m].Data(), strRapHigh[m].Data())<<"     "<<m<<endl;
        hpTMixed[m]->Write();
    ...
    TFile* fTest = new TFile("subtracted_Rap.root", "RECREATE");
    for(Int_t i=0; i<5; i++)
    {

      SetSparseRange(hSparseDimuon, 2, RapLow[i], RapHigh[i] -1e-6, "");
      ...
      cout<<"Signal "<<hSig[i]->GetNbinsX()<<"    Mixed"<<hpTMixed[i]->GetNbinsX()<<endl;

      hSig[i]->Add((TH1F*)hpTMixed[i], -1);
      hSig[i]->SetTitle(Form("Rap_%sto%s", strRapLow[i].Data(), strRapHigh[i].Data()));
      hSig[i]->GetXaxis()->SetRangeUser(2., 15);

      fTest->cd();
      hSig[i]->SetName(Form("Rap_%sto%s", strRapLow[i].Data(), strRapHigh[i].Data()));
      hSig[i]->Write();
    ...
    hTotal->Add(hSig[3]);
    hTotal->Add(hSig[4]);
    hTotal->SetTitle("Rap_-4to-2.5");
    hTotal->SetName("Rap_-4to-2.5");
    hTotal->Write();
