void DrawPlots(TH1* h0, TH1* h1, TH1* h2, TH1*h3, TH1*h4)
{
...
void compute_sys_iterator()
{
    ...
    for(Int_t i=0; i<hRap_init->GetNbinsX();i++)
    {
        ...
        bRapFin[i]  = hRap_4->GetBinContent(i+1);
    ...
    for(Int_t i=0; i<hpT_init->GetNbinsX();i++)
    {
        ...
        bpTFin[i]  = hpT_4->GetBinContent(i+1);
   ...
   /* for(Int_t i=0; i<hCent_init->GetNbinsX();i++)
    ...
    DrawPlots(hpT_init, hpT_1, hpT_2, hpT_3, hpT_4);
    DrawPlots(hRap_init, hRap_1, hRap_2, hRap_3, hRap_4);
