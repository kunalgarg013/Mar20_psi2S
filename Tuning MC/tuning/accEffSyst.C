...
std::vector<TF1 *> getSmearedFunctions(int centMin = 0, int centMax = 10, bool isPt = true, int nRndm = 50)
{
    ...
    for (int irndm = 0; irndm < nRndm; ++irndm)
    {
        ...
        funcs.emplace_back(getSmearedFunc(histo, smearName.Data(), isPt));
    }
    return funcs;
}
//-------------------------------------------------------------------------//
...
void drawUncertainties(std::vector<std::vector<TH1 *>> accEff, TH1 *refAccEffPt, TH1 *refAccEffRap, std::vector<std::vector<TF1 *>> newPtRap, TF1 *refPt, TF1 *refRap, Int_t centMin, Int_t centMax, TString typeUncert)
{
    ...
    for (int itype = 0; itype < 2; ++itype)
    {
        ...
        legend->Draw();
        canFunc->SaveAs(Form("Cent-%dto%d/AccEff%s/InputVariationVs%s.png", centMin, centMax, typeUncert.Data(), var.Data()));
        ...
        legend->Draw();
        canAccEff->SaveAs(Form("Cent-%dto%d/AccEff%s/AccEffVariationVs%s.png", centMin, centMax, typeUncert.Data(), var.Data()));
        ...
        legend->Draw();
        canRatio->SaveAs(Form("Cent-%dto%d/AccEff%s/AccEffVariationOverRefVs%s.png", centMin, centMax, typeUncert.Data(), var.Data()));
        ...
        gr->Draw("ap");
        legend->Draw();
        canRMS->SaveAs(Form("Cent-%dto%d/AccEff%s/AccEffRMSOverMeanVs%s.png", centMin, centMax, typeUncert.Data(), var.Data()));
        if(typeUncert=="Stat")
        {
            gr->SetName(Form("%s", name.Data()));
            gr->Write();
