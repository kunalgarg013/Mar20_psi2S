...
// Double_t rapRanges[][2] = {{-4, -2.5}, {-4, -3.5}, {-3.5, -3.25}, {-3.25, -3}, {-3, -2.75}, {-2.75, -2.5}};
...
void FitAndStore()
{
  TFile *inputFile = new TFile("./signalhistos_cent_pt_rap.root");
  ...
  for (int iCentBin = 0; iCentBin < numberOfCentRanges; iCentBin++)
  {
    for (int iRapBin = 0; iRapBin < numberOfRapRanges; iRapBin++)
    {
      ...
      {
        // TH1D *histoInvmass = ((TH1D *)inputFile->Get(Form("SignalHisto_Cent_%gto%g_pT_%gto%g_Rap%gto%g", centRanges[iCentBin][0], centRanges[iCentBin][1], ptRanges[iPtBin][0], ptRanges[iPtBin][1], rapRanges[iRapBin][0], rapRanges[iRapBin][1])));
        TH1D *histoInvmass = ((TH1D *)inputFile->Get(Form("SignalHisto_Cent_%gto%g_pT_%gto%g_Rap%gto%g", centRanges[iCentBin][0], centRanges[iCentBin][1], ptRanges[iPtBin][0], ptRanges[iPtBin][1], rapRanges[iRapBin][0], rapRanges[iRapBin][1])));
