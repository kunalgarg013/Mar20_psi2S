...
Double_t rapRanges[][2] = {{-4, -3.5}, {-3.5, -3.25}, {-3.25, -3}, {-3, -2.75}, {-2.75, -2.5}};
...
void FitAndStore()
{
  TFile *inputFile = new TFile("./signalHistos_CMUL_combined_Rapidity.root");
  // TFile *inputFile = new TFile("./subtracted_Rap.root");
  ...
  for (int iCentBin = 0; iCentBin < numberOfCentRanges; iCentBin++)
  {
    for (int iRapBin = 0; iRapBin < numberOfRapRanges; iRapBin++)
    {
      for (int iPtBin = 0; iPtBin < numberOfPtRanges; iPtBin++)
      {
        ...
        // TH1D *histoInvmass = ((TH1D *)inputFile->Get(Form("Rap_%gto%g",rapRanges[iRapBin][0], rapRanges[iRapBin][1])));    //pT Mixed event subtracted
        TH1D *histoInvmass = ((TH1D *)inputFile->Get(Form("SignalHisto_Cent_0to90_Rap_%gto%g", rapRanges[iPtBin][0], rapRanges[iPtBin][1])));
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++//
