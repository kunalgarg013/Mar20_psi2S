...
void pt_ratio_jpsi()
{
    ...
    for(Int_t i=0; i<numberOfPtRanges; i++)
    {
        hRatio[i] = new TH1D(Form("%g_%g_Ratio", ptRanges[i][0], ptRanges[i][1]),Form("%g_%g_Ratio", ptRanges[i][0], ptRanges[i][1]), 50, 0, 50 );
    }

    for (int iPtBin = 0; iPtBin < numberOfPtRanges; iPtBin++)
    {
        Int_t iTest= 1;
        rangeName.Form("Cent_0-90AndPt_%g-%gAndRap_-4to-2.5", ptRanges[iPtBin][0], ptRanges[iPtBin][1]);
                ...
                for (int iTails = 0; iTails < numberOfTailsSets; iTails++)
                {
                    ...
                    for (int iFitRange = 0; iFitRange < numberOfFitRanges; iFitRange++)
                    {
                        for (int iPsi2sWidth = 0; iPsi2sWidth < numberOfPsi2sWidth; iPsi2sWidth++)
                        {
                            ...
                            if(rangeName == rangeOfTests[iPtBin])
                            {
                                ...
                                hRatio[iPtBin]->SetBinContent(iTest+1, testFitResults[1]/testFitResults[0]);
                                hRatio[iPtBin]->SetBinError(iTest+1, AbsErr);
                                hRatio[iPtBin]->GetXaxis()->SetBinLabel(iTest + 1, testName);
                                iTest++;
                            }
                        }
                    }
    ...
    TFile* fSave = new TFile("ratio_pt_jpsi.root", "RECREATE");
    fSave->cd();
    hRatio[0]->Write();
    hRatio[1]->Write();
    hRatio[2]->Write();
